...
import { polybotWorkerInjection } from "https://cdn.polymodloader.com/gh/michael201110/poly-bot/v0.1.14/pml-mod/0.1.0/worker_runtime.js";
...
class PolyBotBridgeMod extends PolyMod {
  ...
  preInit = (pml) => {
    ...
    globalThis.__polybotWrapSimulationWorker = (worker) => {
      let finishRestartScheduled = false;
      ...
      worker.addEventListener("message", (event) => {
        if (event.data?.polybotAbortRestart === true) {
          pressBackspace();
          return;
        }
        if (event.data?.polybotPlayerFinished !== true || finishRestartScheduled) {
          return;
        }
        finishRestartScheduled = true;
        setTimeout(() => {
          pressBackspace();
          finishRestartScheduled = false;
        }, 500);
      });
      return worker;
    };
