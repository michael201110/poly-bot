...
export function polybotWorkerInjection() {
  {
    const runtimeKey = "__polybotWorkerRuntime_v2";
    ...
    if (!self[runtimeKey]) {
      self[runtimeKey] = (() => {
        ...
        function decodeState(buffer) {
          ...
          for (let field = 0; field < 4; field += 1) {
            ...
            for (let wheel = 0; wheel < 4; wheel += 1) {
              values.push(finite(view.getFloat32(offset, true)));
              offset += 4;
            }
            wheelValues.push(values);
          }
          const [suspensionLengths, suspensionVelocities, wheelRotations, wheelSkids] = wheelValues;
        ...
        function buildTelemetry(decoded, action, dtSeconds) {
          ...
          // All ghost guidance must describe the nearest point on the racing
          // line. A time-indexed ghost can be hundreds of metres ahead after
          // the learner loses time, producing unrelated controls and poses.
          const timedGuide = guide;
          ...
          const localAcceleration = localVelocity.map((value, index) =>
            dtSeconds > 0 && session?.previousLocalVelocity
              ? finite((value - session.previousLocalVelocity[index]) / dtSeconds)
              : 0,
          );
          const ghostDelta = subtract(timedGuide.position, decoded.position);
          const ghostRelativePosition = [
            dot(ghostDelta, basis.right),
            dot(ghostDelta, basis.up),
            dot(ghostDelta, basis.forward),
          ].map((value) => finite(value));
          const ghostHeadingError = Math.atan2(
            dot(cross(basis.forward, timedGuide.tangent), basis.up),
            clamp(dot(basis.forward, timedGuide.tangent), -1, 1),
          );
          const timedIndex = referenceIndex;
          const speedBefore = reference.points[Math.max(0, timedIndex - 1)];
          const speedAfter = reference.points[Math.min(reference.points.length - 1, timedIndex + 1)];
          const speedFrames = Math.max(1, speedAfter.frame - speedBefore.frame);
          const ghostTargetSpeed =
            distance(speedBefore.position, speedAfter.position) / (speedFrames * fixedDtSeconds);
          ...
          return {
            ...
            local_acceleration_mps2: localAcceleration,
            ...
            suspension_lengths_m: decoded.suspensionLengths,
            suspension_velocities_mps: decoded.suspensionVelocities,
            wheel_skids: decoded.wheelSkids,
            actual_steering: decoded.steering,
            ghost_relative_position_m: ghostRelativePosition,
            ghost_heading_error_rad: finite(ghostHeadingError),
            ghost_target_speed_mps: finite(ghostTargetSpeed),
        ...
        function transition(
          ...
          session.previousLocalVelocity = state.local_velocity_mps;
